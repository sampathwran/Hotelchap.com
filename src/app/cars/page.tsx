    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <div className="relative w-full min-h-[500px] flex items-center justify-center py-12 md:py-0">
        ...
        <div className="relative z-10 w-full max-w-6xl px-4 md:px-8 flex flex-col items-center text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 drop-shadow-lg">{t("Hit the road with HotelChap")}</h1>
          <p className="text-lg md:text-xl text-white/90 font-medium mb-8 drop-shadow-md">{t("Compare the best car and bike rental deals worldwide")}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
            ...
            <h3 className="font-bold text-xl text-gray-900 mb-3">{t("Huge Selection")}</h3>
            <p className="text-gray-500 font-medium leading-relaxed">{t("From economy cars to luxury SUVs and agile scooters, find your perfect ride.")}</p>
          </div>
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
            <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center text-[#673AB7] mb-6">
              <ShieldCheck size={36} />
            </div>
            <h3 className="font-bold text-xl text-gray-900 mb-3">{t("Secure & Insured")}</h3>
            <p className="text-gray-500 font-medium leading-relaxed">{t("Most of our rentals come with basic insurance included for your peace of mind.")}</p>
          </div>
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
            <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center text-[#673AB7] mb-6">
              <MapPin size={36} />
            </div>
            <h3 className="font-bold text-xl text-gray-900 mb-3">{t("Anywhere You Go")}</h3>
            <p className="text-gray-500 font-medium leading-relaxed">{t("Pick up your vehicle from airports, train stations, or downtown city locations.")}</p>
          </div>
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center text-center hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
            <div className="w-20 h-20 bg-purple-50 rounded-full flex items-center justify-center text-[#673AB7] mb-6">
              <HeadphonesIcon size={36} />
            </div>
            <h3 className="font-bold text-xl text-gray-900 mb-3">{t("24/7 Support")}</h3>
            <p className="text-gray-500 font-medium leading-relaxed">{t("Our multi-lingual customer support team is always here to help you on the road.")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
          <h2 className="text-3xl font-black text-gray-900 mb-6">{t("Drive your own adventure")}</h2>
          <p className="text-gray-500 text-lg mb-8 leading-relaxed">{t("Whether you're exploring winding mountain roads on a motorcycle, or cruising along the coastline with your family in a spacious SUV, we have exactly what you need.")}</p>
          <ul className="space-y-4">
            <li className="flex items-center gap-3 text-gray-700 font-bold"><CheckCircle className="text-green-500" size={24} /> {t("Free cancellation on most bookings")}</li>
            <li className="flex items-center gap-3 text-gray-700 font-bold"><CheckCircle className="text-green-500" size={24} /> {t("No hidden credit card fees")}</li>
            <li className="flex items-center gap-3 text-gray-700 font-bold"><CheckCircle className="text-green-500" size={24} /> {t("Flexible pick-up and drop-off")}</li>
          </ul>
        </div>
