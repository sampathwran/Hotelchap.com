      ...
      >
        {offers.map((offer) => {
          ...
          return (
            <div 
              key={offer.id} 
              className={`min-w-full md:min-w-[calc(50%-12px)] lg:min-w-[calc(33.333%-16px)] snap-start shrink-0 bg-gradient-to-r ${design.bgClass} rounded-3xl p-8 relative overflow-hidden shadow-lg transform hover:-translate-y-1 transition duration-300`}
            >
              ...
              <div className="relative z-10 flex flex-col h-full justify-between items-start min-h-[220px]">
                <div>
                  <span className={`${design.textColor === "text-white" ? "bg-white/20" : "bg-black/10"} px-3 py-1 rounded-full text-sm font-bold tracking-widest uppercase mb-4 inline-block ${design.textColor}`}>
                    {offer.badgeText}
                  </span>
                  <h3 className={`text-2xl md:text-3xl font-black mb-3 ${design.textColor}`}>
                    {offer.title}
                  </h3>
                  <p className={`${design.subTextColor} mb-8 max-w-sm font-medium line-clamp-3`}>
                    {offer.description}
                  </p>
                </div>
                ...
                <a 
                  ...
                  className={`${design.buttonBg} ${design.buttonText} font-bold px-6 py-3 rounded-xl shadow-md hover:scale-105 transition-transform mt-auto`}
                >
