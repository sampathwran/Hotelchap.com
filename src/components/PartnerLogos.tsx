    ...
    <div className="w-full px-4 md:px-10 mt-12 md:mt-16 mb-6">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div className="text-center md:text-left min-w-[200px]">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-1">{t("We Compare Prices From")}</h3>
          <p className="text-xl font-extrabold text-gray-800">{t("100+ Travel Sites")}</p>
        </div>

        <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10 transition-all duration-500">
          {partners.map((p, i) => (
            <div key={i} className="flex items-center gap-2">
              <img src={`/api/logo/${p.domain.split('.')[0]}`} alt={p.name} className="w-6 h-6 md:w-8 md:h-8 rounded-full shadow-sm bg-white" />
